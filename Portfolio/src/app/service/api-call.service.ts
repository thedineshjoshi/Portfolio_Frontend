...
export class ApiCallService {
  ...
  private certificateUploadUrl = 'https://localhost:7209/api/Certification';
  ...
  addCertificate(certificate: manageCertificates): Observable<any> {
    return this.http.post(
      `${this.certificateUploadUrl}/AddCertificate`,
      certificate,
      { responseType: 'json' }
    );
  }
  ...
  addTimeline(timeline: Timeline): Observable<any> {
    return this.http.post(`${this.timelineUploadUrl}/AddTimeline`, timeline, {
      responseType: 'json',
    });
  }
  getTimeline(): Observable<Timeline[]> {
    return this.http.get<Timeline[]>(`${this.timelineUploadUrl}/GetTimeline`, {
      responseType: 'json',
    });
  }
  addComment(comment: BlogComment, blogId: string): Observable<any> {
    return this.http.post(
      `${this.blogApiUrl}/AddComment?blogId=${encodeURIComponent(blogId)}`,
      comment,
      { responseType: 'json' }
    );
  }
  getCommentsByBlogId(blogId: string): Observable<any> {
    return this.http.get(`${this.blogApiUrl}/GetCommentsByBlogId/${blogId}`, {
      responseType: 'json',
    });
  }
  // Projects Apis
  ...
  updateProject(id: string, projectPost: ProjectPost): Observable<ProjectPost> {
    return this.http.put<ProjectPost>(
      `${this.projectApiUrl}/UpdateProject/${id}`,
      projectPost,
      { responseType: 'json' }
    );
  }
